    ...
    // --- Custom FPDF Class (Keep as before) ---
     class PDF extends FPDF {
        ...
        function Footer() { 
            ...
            $generatedByStr = 'Generated By: ' . $this->generatedBy . ' on ' . date('Y-m-d H:i'); 
            if (function_exists('iconv')) { $generatedByStr = @iconv('UTF-8', 'cp1252//IGNORE', $generatedByStr) ?: $generatedByStr; } $this->Cell(0, 5, $generatedByStr, 0, 1, 'L'); $this->Cell(0, 5, 'Page ' . $this->PageNo() . '/{nb}', 0, 0, 'C'); }
        ...
        function SummarySection($title, $items) { 
            ...
            $this->SetTextColor(254, 74, 199); 
            $this->Cell(0, 10, $title, 0, 1, 'L'); 
            $this->SetFont('Arial', '', 10); 
            $this->SetTextColor($this->colorDark[0], $this->colorDark[1], $this->colorDark[2]); 
            $this->SetFillColor(255); $this->SetDrawColor($this->colorBorder[0], $this->colorBorder[1], $this->colorBorder[2]); 
            $this->SetLineWidth(.2); $labelWidth = 80; $pageWidth = $this->GetPageWidth() - $this->lMargin - $this->rMargin; 
            $valueWidth = $pageWidth - $labelWidth; $fill = false;
            foreach ($items as $label => $value) { 
                $isTotal = (strpos(strtoupper($label), 'TOTAL') !== false || strpos(strtoupper($label), 'NET') !== false); 
                $this->SetFont('Arial', $isTotal ? 'B' : '', 10); 
                $this->SetFillColor($fill ? $this->colorLightPink[0] : 255, $fill ? $this->colorLightPink[1] : 255, $fill ? $this->colorLightPink[2] : 255); 
                $labelConverted = function_exists('iconv') ? (@iconv('UTF-8', 'cp1252//IGNORE', $label) ?: $label) : $label; $valueFormatted = number_format($value, 2); $valueConverted = function_exists('iconv') ? (@iconv('UTF-8', 'cp1252//IGNORE', '₱ '.$valueFormatted) ?: ('₱ '.$valueFormatted)) : ('₱ '.$valueFormatted); $this->Cell($labelWidth, 8, $labelConverted, 1, 0, 'L', true); $this->Cell($valueWidth, 8, $valueConverted, 1, 1, 'R', true); $fill = !$fill; } $this->SetFont('Arial', '', 10); $this->Cell($labelWidth + $valueWidth, 0, '', 'T'); $this->Ln(8); }
    }
    ...
    $main_title = '';
     if ($report_type === 'collections') {
        // Collections report remains the same, showing individual collection entries
         ...
         $pdf->Cell(0, 5, ucfirst($report_type), 0, 1, 'C'); 
         ...
         if (!empty($table_data)) { 
            ...
            $pdf->SetFont('Arial', '', 10); 
            $pdf->Cell(0, 10, 'No collection data found.', 0, 1, 'C'); 
        }
     ...
     elseif ($report_type === 'expenses') {
        ...
        $pdf->setReportHeader($main_title, $period_label, $user_name);
         ...
         if (!empty($table_data)) { 
            ...
            $pdf->BasicTable($header, $table_data); $pdf->Ln(5); 
            $pdf->SetFont('Arial', 'B', 10); 
            $pdf->Cell(0, 7, 'Total Expenses: ' . number_format($total_expenses, 2), 0, 1, 'R');
        }
         else { 
            $pdf->SetFont('Arial', '', 10); 
            $pdf->Cell(0, 10, 'No expense data found.', 0, 1, 'C'); }
