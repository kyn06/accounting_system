...
class PDF extends FPDF {
    ...
    function Footer() { 
      ...
      if (function_exists('iconv')) { 
        $generatedByStr = @iconv('UTF-8', 'cp1252//IGNORE', $generatedByStr) ?: $generatedByStr; } $this->Cell(0, 5, $generatedByStr, 0, 1, 'L'); $this->Cell(0, 5, 'Page ' . $this->PageNo() . '/{nb}', 0, 0, 'C'); }
    
    function BasicTable($header, $data) {
        $this->SetFillColor($this->colorAccent[0], $this->colorAccent[1], $this->colorAccent[2]);
        $this->SetTextColor(255);
        $this->SetDrawColor(max(0, $this->colorAccent[0]-20), max(0, $this->colorAccent[1]-20), max(0, $this->colorAccent[2]-20));
        $this->SetLineWidth(.3);
        $this->SetFont('', 'B', 9);
        ...
        foreach ($data as $row) {
            ...
            $detailsText = $row[$detailsIndex] ?? '';
            $detailsText = str_replace("₱", "PHP ", $detailsText);
            $detailsWidth = $widths[$detailsIndex];

            $lines = $this->NbLines($detailsWidth, utf8_decode($detailsText));
            $rowHeight = $lines * $cellHeight;
